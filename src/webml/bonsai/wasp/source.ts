 ...
 *
 * WASP SOURCE — the network half of the weight-streaming plane.
 *
 * Streams a remote file via HTTP range requests so we NEVER download the whole ~3.8 GB to
 * read a header, and never hold more than the caller asked for. A growable window is
 * extended on underflow. `fetchRange` is injectable so every consumer is unit-testable in
 * Node against an in-memory buffer (no network).
 *
 * MOVED HERE from `gguf/reader.ts`, which remains as a re-export: none of this is a GGUF
 * concern. It will stream any file from any host that answers Range with CORS, which is
 * exactly what `awasp` claims to do for a stranger. See `gguf/reader.ts` for why that
 * shim is not deleted (a published subpath product repos import BY PATH).
 */
